...
(function (global) {
  ...
  const TESTIMONIALS = [
    ...
    },
    {
      ...
      quote: "Closed 2 clients from one search. Best ₦25k I spent.",
    },
  ];
  ...
  function scenePricing(ctx, t) {
    ...
    if (step(2) > 0) {
      ...
      if (strikeP < 1) {
        drawText(ctx, "₦75,000", W / 2, y, 68, "rgba(244,244,255,0.3)", FONT_HEAD);
        setFont(ctx, 68, FONT_HEAD, "700");
        const sw = ctx.measureText("₦75,000").width;
        ctx.strokeStyle = C.red;
        ctx.lineWidth = 4;
        ctx.beginPath();
      ...
      if (smoothstep(2000, 2500, local) > 0) {
        drawText(ctx, "₦25,000", W / 2, y, 96, C.white, FONT_HEAD);
      }
      y += 108;
    }
    if (step(3) > 0) {
    ...
    if (smoothstep(2800, 3100, local) > 0) {
      ...
      ctx.fillStyle = sg;
      ctx.fill();
      drawText(ctx, "Get Lifetime Access — ₦25,000", W / 2, btnY + 34, 24, C.white, FONT_BODY, "center", "700");
    }
    ctx.globalAlpha = 1;
  }
