...
export default function AdminPage() {
  ...
  async function handleSendDirectMessage() {
    if (!msgSubject.trim()) {
      setMsgResult("Subject line is required.");
      return;
    }
    if (!msgBody.trim()) {
      setMsgResult("Message body is required.");
      return;
    }
    if (msgMode === "single" && !msgRecipient.trim()) {
      setMsgResult("Recipient email is required.");
      return;
    }
    ...
    const confirmed = window.confirm(
      msgMode === "broadcast"
        ...
        : `Send "${msgSubject}" to ${msgRecipient}?`
    );
    if (!confirmed) return;
    ...
    try {
      ...
      const res = await fetch(`${apiUrl}/admin/send-message`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
      ...
      if (res.ok && data.success) {
        setMsgResult(data.message || "Message sent successfully");
        setMsgSubject("");
        setMsgBody("");
        setMsgRecipient("");
      } else {
        setMsgResult(data.error || "Failed to send message.");
      }
    } catch {
        ...
        >
          ...
          <div style={{ padding: 16 }}>
            <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
              {(["single", "broadcast"] as const).map((mode) => (
                <button
                  ...
                  style={{
                    background: msgMode === mode ? "#7C3AED" : "rgba(255,255,255,0.05)",
            ...
            {msgMode === "broadcast" && (
              ...
              >
                This will send to all activated users. Double check your message before sending.
              </div>
            )}
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <p style={{ fontSize: 11, color: "#555570", marginBottom: 8 }}>
                Use double line breaks between paragraphs. Each paragraph becomes a separate block in
                the email.
              </p>
              <textarea
                value={msgBody}
            ...
            <div style={{ marginBottom: 16 }}>
              <button
                type="button"
                onClick={() => setMsgPreview(!msgPreview)}
                style={{
                  background: "transparent",
                  color: "#A78BFA",
              ...
              >
                {msgPreview ? "Hide Preview" : "Preview Email Structure"}
              </button>
            </div>
            ...
            {msgPreview && (
              ...
              >
                ...
                >
                  ...
                  {msgBody
                    ? msgBody.split("\n\n").filter(Boolean).map((para, i) => (
                        <p key={i} style={{ marginBottom: 12, fontSize: 13, color: "#444444" }}>
                          {para}
                        </p>
                      ))
                    : (
                ...
                >
                  Lead<span style={{ color: "#7C3AED" }}>Thur</span> &middot; access@leadthur.com
                  &middot; leadthur.com
                </div>
              </div>
            )}
            ...
            {msgResult && (
              <div
                style={{
                  ...
                  padding: "10px 14px",
                  background:
                    msgResult.includes("success") ||
                    msgResult.includes("sent") ||
                    msgResult.includes("Sent")
                      ? "rgba(16,185,129,0.08)"
                      : "rgba(239,68,68,0.08)",
                  border: `1px solid ${
