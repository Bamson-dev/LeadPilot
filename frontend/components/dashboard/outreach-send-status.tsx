  ...
  return (
    <div className="glass rounded-2xl p-4 sm:p-6">
      ...
      {loading && sends.length === 0 ? (
        <p className="mt-4 text-sm text-[#6B6B80]">Loading recent sends…</p>
      ) : sends.length === 0 ? (
        <p className="mt-4 text-sm text-[#6B6B80]">
          No emails sent yet. Select leads with an email and use Send email above.
        </p>
      ) : (
            ...
            <tbody>
              {sends.map((row) => (
                <tr key={row.id} className="border-b border-white/[0.04]">
                  ...
                  <td className="py-2.5 pr-3 text-[#A1A1B5] max-w-[200px] truncate">
                    {row.subject || "—"}
                  </td>
