-- Content automation hardening: 3-hour publishing interval, local image storage settings,
-- scheduler heartbeat columns. Additive only — does not modify existing rows destructively.
...
-- Initialize interval only when missing (preserve existing production settings).
update content_automation_settings
set publishing_interval_hours = 3
where id = 1
  and publishing_interval_hours is null;
